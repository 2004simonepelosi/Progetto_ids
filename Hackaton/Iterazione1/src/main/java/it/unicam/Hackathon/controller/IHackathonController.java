...
public class IHackathonController {
    ...
    public ResponseEntity<Map<String, Object>> creaHackathon(@RequestBody CreaHackathonRequest dto) {
        ...
        return ResponseEntity.ok(Map.of(
                ...
                "ruoloOrganizzatore", h.getOrganizzatore().getRuolo(),
                "stato", h.getStato()
        ));
    }

    @PostMapping("/{id}/giudice")
    public ResponseEntity<Map<String, Object>> nominaGiudice(
            @PathVariable Long id, @RequestParam Long utenteId) {
        ...
        return ResponseEntity.ok(Map.of(
                ...
                "ruolo", h.getGiudice().getRuolo(),
                "hackathon", h.getNome()
        ));
    }

    @PostMapping("/{id}/mentore")
    public ResponseEntity<Map<String, Object>> nominaMentore(
            @PathVariable Long id, @RequestParam Long utenteId) {
        Hackathon h = gestoreHackathon.nominaMentore(id, utenteId);
        return ResponseEntity.ok(Map.of(
                "messaggio", "Mentore nominato con successo!",
                "hackathon", h.getNome(),
                "numeroMentori", h.getMentori().size()
        ));
    }

    @PostMapping("/{id}/vincitore")
    public ResponseEntity<Map<String, Object>> proclamaVincitore(
        ...
        return ResponseEntity.ok(Map.of(
                ...
                "teamVincitore", h.getVincitore().getNome(),
                "stato", h.getStato()
        ));
    }
}